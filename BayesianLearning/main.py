...
def create_bayes_classifier(training_data):
    ...
    for i in range(0, len(probabilities_c1)-1):
        bayes_classifier_c1[i] = (round(1 - probabilities_c1[i], 2), round(probabilities_c1[i], 2))
    bayes_classifier_c0 = [(0, 0) for _ in range(len(probabilities_c0))]
    for i in range(0, len(probabilities_c0)-1):
        bayes_classifier_c0[i] = (round(1 - probabilities_c0[i], 2), round(probabilities_c0[i], 2))
    
    #Calculate P(class=0) P(class=1)
    bayes_classifier_c1[len(probabilities_c1)-1] = (0, round(frequencies_c1[len(frequencies_c1)-1] / len(training_data), 2))
    bayes_classifier_c0[len(probabilities_c0)-1] = (0, round(frequencies_c0[len(frequencies_c0)-1] / len(training_data), 2))
    #Returns list of 
...
def print_bayes_classifier(bayes_classifier, labels):
    ...
    for i in range(0, len(bayes_classifier)):
        ...
        print(f"P(" + str(labels[len(labels)-1]) + "=" + str(i) + ")=" + str(bayes_classifier[i][len(bayes_classifier[i])-1][1]) + " ", end="")
...
def main():
    ...
    training_accuracy = evaluate_classifier()
